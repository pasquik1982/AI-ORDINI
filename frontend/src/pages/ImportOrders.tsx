...
import { client, ParsedSupplierRow } from '../lib/api';
import { Upload, CheckCircle, AlertCircle } from 'lucide-react';

export default function ImportOrders() {
  const [file, setFile] = useState<File | null>(null);
  const [parsedData, setParsedData] = useState<ParsedSupplierRow[]>([]);
    ...
    try {
      ...
      reader.onload = async (event) => {
        ...
        const response = await client.apiCall.invoke({
          url: '/api/v1/upload/parse-suppliers',
          method: 'POST',
          data: {
  ...
  const importData = async () => {
    ...
    try {
      ...
      const response = await client.apiCall.invoke({
        url: '/api/v1/upload/import-suppliers',
        method: 'POST',
        data: {
          rows: parsedData,
        },
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-3xl font-bold text-gray-900">Import Ordini Fornitori</h1>
        <p className="text-gray-600 mt-2">
          Carica file CSV o Excel contenenti gli ordini dai fornitori
        </p>
      </div>
                ...
                <table className="w-full text-sm">
                  <thead>
                    <tr className="border-b bg-gray-50">
                      ...
                      <th className="text-left p-2">Data Ordine</th>
                  ...
                  <tbody>
                    {parsedData.slice(0, 10).map((row, idx) => (
                      <tr key={idx} className="border-b hover:bg-gray-50">
                        <td className="p-2">{row.fornitore}</td>
                        <td className="p-2">{new Date(row.data_ordine).toLocaleDateString('it-IT')}</td>
                        <td className="p-2">{new Date(row.data_consegna).toLocaleDateString('it-IT')}</td>
                        <td className="p-2">{row.categoria_prodotto}</td>
                        <td className="text-right p-2">{row.quantita}</td>
                        <td className="text-right p-2">€{row.prezzo_totale.toFixed(2)}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
